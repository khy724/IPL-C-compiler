...
%code{
   ...
   stack<map<string,string>> Temp_Stack;
   stack<string> Act_record;
   map<string,string> goto_label;
   int lccount=0;
   int N=6;
...
main_definition:
 INT {localtb = new SymbTab(); funtype = "int"; offset = PARAM; }
 MAIN '(' ')' {
...
fun_declarator: 
      IDENTIFIER '(' {scope = "param";} parameter_list ')' 
      {
            ...
            for(auto & it : $4->list) {
                  ...
                  localtb->Entries[it->name]->ret_type = it->type;
...
declarator_arr: 
      ...
      | declarator_arr '[' INT_CONSTANT ']' 
      {
            ...
            size_map.insert({local_type->type, local_type->size});
            // cout<<"2"<<local_type->type<<local_type->size<<endl;
...
declarator: 
      ...
      | '*' {local_type->type = local_type->type+ "*";local_type->size = 4; size_map.insert({local_type->type, local_type->size});
            // cout<<"3"<<local_type->type<<local_type->size<<endl;
            } declarator 
      {
            // $1->derefLevel++;
...
printf_call
  : PRINTF '(' STRING_LITERAL ')' ';'{
        localtb->Localstr.insert({$3,"LC"+to_string(lccount)});
            lccount++;
...
postfix_expression: 
      ...
      {
            $$ = new arrayref_astnode($1,$3, &size_map);
            $$->isaddr = $1->isaddr;
            if($3->typeval!="int" ){
...
primary_expression:
      ...
      {  
            if(localtb->Entries.find($1)==localtb->Entries.end()){
               ...
               int ac = count(l.begin(),l.end(),'[');
               if(localtb->Entries[$1]->global=="param"){
                     $$->isparam=true;
               }
               
               if(localtb->Entries[$1]->global=="param" && strc==0 && ac==0){
                     $$->isaddr=true;
               }
               
               if(localtb->Entries[$1]->global=="local" && strc==0){
                  $$->isaddr=true;
               }
      ...
      | STRING_LITERAL 
      {
            ...
            $$->typeval = "string";
            $$->lvalue = false;
            localtb->Localstr.insert({$1,"LC"+to_string(lccount)});
            lccount++;
      }
      | '(' expression ')' 
      {
...
declarator_list: 
      ...
      local_type = new type_specifier_class(base_type->type,base_type->size);} declarator 
      {
            ...
            size+=local_type->size;
            size_map.insert({local_type->type, local_type->size});
            // cout<<"4"<<local_type->type<<local_type->size<<endl;
      ...
      | declarator_list ',' {scope = "local"; local_type = new type_specifier_class(base_type->type,base_type->size);} declarator 
      {
            ...
            Entry *a;
            size_map.insert({local_type->type, local_type->size});
            // cout<<"1"<<local_type->type<<local_type->size<<endl;
            if(fname != "@$") a = new Entry("var", "local", local_type->type, local_type->size, offset);
            else a = new Entry("var", "local", local_type->type, local_type->size, size);
            size+=local_type->size;
            if(local_type->type=="void"){
